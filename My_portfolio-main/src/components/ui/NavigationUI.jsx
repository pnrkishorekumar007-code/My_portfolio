...
const NavigationUI = () => {
    ...
    }, [hoveredRoom, currentRoom, paintedMapsRefs.gallery, paintedMapsRefs.contact, paintedMapsRefs.about, paintedMapsRefs.studio]);
    ...
    // Keyboard shortcuts: Ctrl/Cmd+K for map, 1-4 for quick teleport
    useEffect(() => {
        const handleKeyboard = (e) => {
            // Ctrl/Cmd + K: Toggle map
            ...
            // Number keys 1-4 for quick teleport (only when not in a room)
            if (!isInRoom && !isTeleporting && hasEntered) {
                const roomMap = { '1': 'about', '2': 'gallery', '3': 'contact', '4': 'studio' };
                if (roomMap[e.key]) {
                    e.preventDefault();
                    teleportTo(roomMap[e.key]);
                }
            }
        };
        window.addEventListener('keydown', handleKeyboard);
        return () => window.removeEventListener('keydown', handleKeyboard);
    }, [isInRoom, isTeleporting, hasEntered, teleportTo]);
