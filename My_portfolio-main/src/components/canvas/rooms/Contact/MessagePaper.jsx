...
const FONT_PATH = '/fonts/CabinSketch-Regular.woff';

// Email validation helper (pure)
...
// Web3Forms API Key â€” loaded from environment variable so it's not exposed in the repo.
// Set VITE_WEB3FORMS_KEY in .env (local dev) and in Cloudflare Pages dashboard (production).
const WEB3FORMS_KEY = import.meta.env.VITE_WEB3FORMS_KEY || '';

// Fetch with abort-based timeout so a stalled connection can't leave the
// form stuck on "SENDING..." forever. Aborts surface as AbortError, which
// the existing try/catch blocks already handle.
...
const ALLOWED_ORIGINS = [
    'kishorekumar.dev',
    'www.kishorekumar.dev',
    'vercel.app', // matches any *.vercel.app preview/production URL
    'localhost',
    '127.0.0.1',
];
...
// Common bigrams in English + Polish â€” real words use these frequently.
...
const COMMON_BIGRAMS = new Set([
    ...
    'ie','rz','sz','cz','ni','na','po','prz','od','do','Å›Ä‡','za','ko','ow',
    'sk','st','mi','wy','dz','ka','ra','je','ro','em','os','ak','ek','go',
    'aÅ‚','ze','cz','rz','ja','ma','ci','Å‚o','wa','da','no','mo','li','iÄ‡',
]);
...
const scoreWord = (word) => {
    ...
    if (/^[^a-zA-ZÄ…Ä‡Ä™Å‚Å„Ã³Å›ÅºÅ¼Ä„Ä†Ä˜ÅÅƒÃ“ÅšÅ¹Å»]+$/.test(word)) return 1.0; // Non-alpha (e.g. "!!!")
    ...
    for (let i = 0; i < lower.length - 1; i++) {
        ...
        if (/[a-zÄ…Ä‡Ä™Å‚Å„Ã³Å›ÅºÅ¼]{2}/.test(bigram)) {
            total++;
    ...
    const vowels = (lower.match(/[aeiouyÄ…Ä™Ã³]/g) || []).length;
    const alpha = (lower.match(/[a-zÄ…Ä‡Ä™Å‚Å„Ã³Å›ÅºÅ¼]/g) || []).length;
    ...
    const hasHugeCluster = /[bcdfghjklmnpqrstvwxzÅ¼ÅºÄ‡]{4,}/i.test(lower);
    const clusterPenalty = hasHugeCluster ? 0.5 : 1.0;

    return bigramScore * vowelPenalty * clusterPenalty;
};

// Main content analyzer â€” scores every word and computes an aggregate
const analyzeContentAI = (text, isSubject = false) => {
    ...
    if (!isSubject && cleaned.length <= 15 && !cleaned.includes(' ')) {
        ...
        const commonShort = /^(hi|hey|hello|thanks|thank you|cool|nice|ok|okay|yes|no|sup|yo|cheers|hej|czeÅ›Ä‡|dziÄ™ki|siema|elo)$/i;
    ...
    const scorableWords = words.filter(w => w.length > 2 && /[a-zA-ZÄ…Ä‡Ä™Å‚Å„Ã³Å›ÅºÅ¼Ä„Ä†Ä˜ÅÅƒÃ“ÅšÅ¹Å»]/.test(w));
    ...
    const allAlpha = (cleaned.toLowerCase().match(/[a-zÄ…Ä‡Ä™Å‚Å„Ã³Å›ÅºÅ¼]/g) || []);
    const allVowels = (cleaned.toLowerCase().match(/[aeiouyÄ…Ä™Ã³]/g) || []);
...
const MessagePaper = ({ position = [0, 0.05, 2], onSend }) => {
    ...
    const handleButtonClick = useCallback(async () => {
        ...
        try {
            ...
            if (timeOnForm < 3000) {
                // Bots submit instantly â€” silently fake success
            ...
            if (domain) {
                try {
                    const dnsRes = await fetchWithTimeout(`https://cloudflare-dns.com/dns-query?name=${domain}&type=MX`, {
    ...
    }, [message, email, subject, onSend, validateForm, botcheck]);
