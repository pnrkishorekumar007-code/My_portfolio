 ...
 */
const RoomWarmup = ({ onWarmupComplete, isLowTier, mountRooms = true }) => {
    ...
    useFrame(() => {
        ...
        if (frameCount.current >= targetFrames) {
            ...
            const finishWarmup = () => {
                // console.info(`🔥 GPU/Shader Warmup Complete ${isLowTier ? '(Bypassed for LOW tier)' : ''}`);
    ...
    // Do not mount rooms at all on low/mid devices to prevent WebGL Context Lost
    // and to cut boot-time GPU upload cost. HIGH tier keeps the full warmup.
    if (isLowTier || !mountRooms) return null;
