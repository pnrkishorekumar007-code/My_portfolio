...
const useInfiniteCamera = ({
    ...
    // Gyroscope listener bookkeeping: prevents double-adds (effect attempt +
    // UI-button call) and post-unmount adds from the async iOS permission path
    const gyroListenerActive = useRef(false);
    const gyroAddCancelled = useRef(false);
    ...
    const requestGyroscopePermission = useCallback(async () => {
        const grant = () => {
            if (gyroAddCancelled.current) return; // unmounted mid-await
            useGyroscope.current = true;
            if (!gyroListenerActive.current) {
                gyroListenerActive.current = true;
    ...
    useEffect(() => {
        ...
        gyroAddCancelled.current = false;
        requestGyroscopePermission();

        return () => {
            gyroAddCancelled.current = true;
            scrollObserver.kill();
            window.removeEventListener('mousemove', handleMouseMove);
            window.removeEventListener('keydown', handleKeyDown);
            window.removeEventListener('deviceorientation', handleDeviceOrientation);
            gyroListenerActive.current = false;
        };
