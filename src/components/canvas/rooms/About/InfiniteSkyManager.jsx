...
// PokÃ³j About jest na Z = -25, wiÄ™c -25 to drzwi pokoju
// -27 = 2 metry za drzwiami (w gÅ‚Ä…b pokoju) - musi matchowaÄ‡ CORRIDOR_CLIP_Z w SkyChunk
...
const IntroMilestone = ({ z, scrollProgressRef }) => {
    ...
    const avatarWidth = 6; // ZwiÄ™kszony rozmiar awatara na chmurce
    ...
    useFrame((state) => {
        ...
        // === TWARDA LINIA CLIP (RÄ˜CZNE OBLICZENIE WORLD Z) ===
        // worldZ = pokÃ³j(-25) + scrollProgress + lokalna pozycja milestone
        const scrollProgress = scrollProgressRef?.current || 0;
        ...
        // === EDYTUJ TUTAJ (INTRO) ===
        // ZwiÄ™ksz rÃ³Å¼nicÄ™ miÄ™dzy Start a End, Å¼eby animacja byÅ‚a wolniejsza
        const spreadStart = -70; // Startuje wczeÅ›niej
        const spreadEnd = -50;   // KoÅ„czy pÃ³Åºniej
        let spreadFactor = 0;
    ...
    return (
        <group ref={groupRef} position={[0, 0, z]}>
            ...
            <Text
                ...
                anchorX="center"
                anchorY="middle"
                font="/fonts/RubikScribble-Regular.woff"
            >
                KISHOREKUMAR R
            </Text>
            ...
            <Text
                ...
                anchorX="center"
                anchorY="middle"
                font="/fonts/CabinSketch-Regular.woff"
            >
            ...
            <Text
                ...
                fontStyle="italic"
            >
                "Crafting end-to-end web applications
            </Text>
            ...
            <Text
                ...
                fontStyle="italic"
            >
                from Django APIs to React interfaces"
            </Text>
        </group>
    );
};
...
/**
 * AWARDS DATA â?? Core skills, dev capabilities, leadership, and creative work
 */
const AWARDS_DATA = {
    featured: {
        id: 'award-featured',
        layout: 'certificate_grid',
        title: 'Core Skills & Highlights',
        items: [
            { label: 'Full-Stack Development (Python/Django)', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: 'Responsive Frontends (HTML/CSS/JS)', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: 'REST APIs & DOM Manipulation', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: 'Git & GitHub Version Control', date: 'Active', image: '/textures/about/button.webp', url: '#' },
        ],
        platformConfig: {
            label: 'EXPERTISE',
            color: '#311059',
    ...
    sotd: {
        ...
        items: [
            { label: 'Django Backend Development', date: '2025', image: '/textures/about/button.webp', url: '#' },
            { label: 'Bootstrap & Responsive UI', date: '2025', image: '/textures/about/button.webp', url: '#' },
            { label: 'Full-Stack Ownership, Solo Builds', date: '2025', image: '/textures/about/button.webp', url: '#' },
            { label: 'Debugging & Optimization', date: 'Active', image: '/textures/about/button.webp', url: '#' }
        ],
        platformConfig: {
            label: 'DEVELOPER',
            color: '#311059',
    ...
    sotm: {
        ...
        items: [
            { label: 'CodeWave 2026 Hackathon — Core Team', date: '2026', image: '/textures/about/button.webp', url: '#' },
            { label: 'ByteBlitz 2026 — Organizing Team', date: '2026', image: '/textures/about/button.webp', url: '#' },
            { label: 'Intercollegiate Technical Symposiums', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: '100+ Participant Hackathon Coordination', date: '2026', image: '/textures/about/button.webp', url: '#' }
        ],
        platformConfig: {
            label: 'ORGANIZER',
            color: '#311059',
    ...
    other: {
        ...
        items: [
            { label: 'Pencil Sketching & Portrait Art', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: 'Manga Illustration', date: 'Active', image: '/textures/about/button.webp', url: '#' },
            { label: 'Published via Thunder Boys Startup', date: '2025', image: '/textures/about/button.webp', url: '#' }
        ],
        platformConfig: {
            label: 'CREATOR',
            color: '#311059',
...
const AwardsMilestone = ({ z, scrollProgressRef }) => {
    // Pobieranie danych nagrÃ³d z Sanity (z fallbackiem)
...
const JourneyMilestone = ({ z, scrollProgressRef }) => {
    ...
    useFrame((state) => {
        ...
        const revealStart = -100; // WczeÅ›niejszy start
        const revealEnd = -20;
        let revealFactor = 0;
    ...
    return (
        <group ref={groupRef} position={[0, 0, z]}>
            ...
            <group ref={freelanceRef} position={[3.5, -2, 0.5]}>
                ...
                {/* CAPTION - EDIT HERE.
                    NOTE: the freelancewyspa.webp artwork has "FREELANCE" carved into the
                    image itself, which doesn't match Kishore's actual experience (an
                    internship, not freelance work). The caption below has been corrected,
                    but the island graphic still visually reads "FREELANCE" until it's
                    replaced with new artwork (e.g. "INTERNSHIP"). */}
                <Text
...
// === EDYTUJ WYSOKOÅšÄ† TUTAJ (zmieÅ„ wartoÅ›Ä‡ 'y' dla kaÅ¼dego balona) ===
...
const SkillBalloon = ({ config, revealFactorRef, spreadFactorRef, timeRef }) => {
    ...
    // === RESPONSYWNOÅšÄ† ===
    // Na mobile (wÄ…ski viewport) balony sÄ… bliÅ¼ej Å›rodka
    const positionScale = isTouch ? 0.5 : 1; // Jak bardzo Å›ciskamy pozycje na mobile
    const spreadScale = isTouch ? 0.4 : 1;   // Jak bardzo zmniejszamy spread na mobile
    const sizeScale = isTouch ? 0.85 : 1;    // TrochÄ™ mniejsze balony na mobile
    ...
    useFrame((state, delta) => {
        if (hovered && !isPopping) {
            targetScale.current = 1.05; // lekkie powiÄ™kszenie
        } else {
    ...
    // Floating animation with unique phase â€” now computed inside useFrame
            ...
            <group ref={innerGroupRef}>
                ...
                {isPopping && textFadeRef.current > 0.01 && (
                    <Text
                        ...
                        maxWidth={{ large: 6, medium: 5, small: 3.5 }[config.size] || 5}
                        textAlign="center"
                        color="#1a1a1a"
                        anchorX="center"
                        anchorY="middle"
...
const SkillsMilestone = ({ z, scrollProgressRef }) => {
    ...
    useFrame((state) => {
        ...
        // Im bliÅ¼ej kamery, tym bardziej balony siÄ™ rozsuwajÄ…
        // WiÄ™kszy zakres = dÅ‚uÅ¼sza, bardziej widoczna animacja
        const spreadStart = -70; // Kiedy animacja SIÄ˜ ZACZYNA (WczeÅ›niej)
        const spreadEnd = -40;    // Kiedy animacja jest PEÅNA (PÃ³Åºniej)
