...
// Edit these values to change the volume and audibility range of sea noise
// ============================================
const AUDIO_SETTINGS = {
    volume: 2,
    distance: 2,           // Distance at which sound starts to fade (refDistance)
    rolloff: 1.2           // Fade speed (rolloffFactor)
};

// ============================================
// ⚙️ LATARNIA SETTINGS - TWEAK HERE
// Edit these values to change the position, rotation, and size of the lighthouse
// ============================================
const LATARNIA_SETTINGS = {
    // Position: [left/right (X), up/down (Y), back/front (Z)]
    position: [-10, 5, -20],

    // Rotation: [tilt forward/back (X), turn left/right (Y), tilt sideways (Z)]
    rotation: [0, 0.1, 0],
...
// Edit these values to change the position, rotation, and size of the ship
// ============================================
...
const ContactRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    useFrame((state, delta) => {
        ...
        if (statekRef.current) {
            ...
            const sailSpeed = 0.04; // much slower sailing (was 0.15)
            const sailAmplitude = 12; // smaller sailing area so it doesn't go off screen (was 25)
    ...
    return (
        <group ref={groupRef} position={[0, -0.7, -5]}>
            ...
            <SocialBarrel
                ...
                label="LINKEDIN"
                onClick={() => window.open('https://www.linkedin.com/in/kishore007kumar', '_blank')}
            ...
            <SocialBarrel
                ...
                texturePath="/textures/contact/beczka.webp"
                label="GITHUB"
                onClick={() => window.open('https://github.com/pnrkishorekumar007-code', '_blank')}
                paintOnBeforeCompile={onBeforeCompile}
                paintUniforms={uniformsData}
            />
            {/* PHONE */}
            <SocialBarrel
                ...
                texturePath="/textures/contact/beczka.webp"
                label="PHONE"
                onClick={() => window.open('tel:+9123505027', '_self')}
                paintOnBeforeCompile={onBeforeCompile}
                paintUniforms={uniformsData}
            />
            {/* EMAIL */}
            <SocialBarrel
                ...
                texturePath="/textures/contact/beczka.webp"
                label="EMAIL"
                onClick={() => window.open('mailto:pnrkishorekumar007@gmail.com', '_self')}
                paintOnBeforeCompile={onBeforeCompile}
                paintUniforms={uniformsData}
            />
            {/* RESUME */}
            <SocialBarrel
                position={isMobile ? [0, 0.8, -8] : [0, 0.8, -9]}
                rotation={[0, 0, 0]}
                texturePath="/textures/contact/beczka.webp"
                label="RESUME"
            ...
            */}
            <group visible={!showSelection}>
                <MessagePaper
                    position={[0, 0.07, 2]}
                    onSend={(_data) => {
                        // console.log('📬 Contact form submitted:', _data);
