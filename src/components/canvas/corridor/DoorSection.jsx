...
const DoorSection = ({
    ...
    const [_roomReady, setRoomReady] = useState(false); // Room signaled it's ready
    ...
    const { _unlockAchievement } = useAchievements();
    ...
    const doorBbTexLeft = useMemo(() => {
        ...
        return tex;
    }, [baseboardTexture, doorBoardWidth, NATURAL_TILE_W]);
            ...
            <group ref={groupRef}>
                ...
                {/* ===================================================
                    ...
                    position={[wallOffsetX - 0.9, 0, 0.02]}
                      - wallOffsetX - 0.9 → distance from door center (increase 0.9 = further from door)
                      - Y = 0             → height (0 = wall center, + = higher, - = lower)
                    scale={[0.5, 0.5, 1]} → arrow size
                    =================================================== */}
                <mesh
                ...
                {/* ===================================================
                    ...
                    position={[wallOffsetX + 0.9, -0.3, 0.02]}
                      - wallOffsetX + 0.9 → distance from door center (right side)
                      - Y = -0.3          → slightly lower than the left arrow
                    scale={[-0.5, 0.5, 1]} → negative X = mirrored reflection
                    =================================================== */}
                <mesh
                ...
                {/* === THRESHOLD STRIPE (threshold under side door) === */}
                {(() => {
                    // Threshold lies on the floor, perpendicular to the side wall
                    // Threshold width = width of the door opening (~1.1)
                    const THRESH_W = 1.1;   // Width (along local X = along the wall)
                    const THRESH_D = 0.15;  // Depth (along local Z = into the corridor)
                ...
                <group position={[wallOffsetX, -0.4, 0]}>
                    ...
                    <group position={[0, doorHeight / 2 + 0.45, 0.08]}>
                        {/* 
                            SIGN SIZE:
                            Change the numbers in args={[Width, Height]}
                            Currently: 1.3 width, 0.65 height
                        */}
                        <mesh>
